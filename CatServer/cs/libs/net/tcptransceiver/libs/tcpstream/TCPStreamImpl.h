...
class TCPStreamImpl: public TCPStream {
    ...
    private:
        ...
        std::string peer_ip;
        int peer_port;
        TCPStreamImpl(int sd_, struct sockaddr_in * addr): sd(sd_), peer_port(ntohs(addr->sin_port)) {
            char ip[50];
            inet_ntop(PF_INET, (struct in_addr*)&(addr->sin_addr.s_addr),
                    ip, sizeof(ip)-1);
            peer_ip = ip;
        };
      
        TCPStreamImpl(const TCPStreamImpl & stream) = default;
};
